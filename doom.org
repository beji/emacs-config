...
(doom! :input
       ...
       (org +pomodoro +roam2 +passwords)               ; organize your plain life in plain text
...
** Spell- and grammarchecking

Langtool defaults to en-US, de-DE should be the default

#+begin_src emacs-lisp
(setq! langtool-default-language "de-DE")
#+end_src

Add custom functions to change languages around
...
The line length for the summary is a bit short
...
(with-system darwin
             (org-add-agenda-custom-command
              '("c" "Jira issues assigned to me"
                ((ejira-jql "assignee = currentUser() AND resolution = Unresolved AND status != Geschlossen order by updated DESC"
                            ((org-agenda-overriding-header "Assigned to me")))
                 (ejira-jql "reporter = currentUser() AND resolution = Unresolved AND status != Geschlossen order by updated DESC"
...
Password management in org, powered by [[https://github.com/alfaromurillo/org-passwords.el][org-passwords.el]], what could possibly go wrong?

#+begin_src emacs-lisp
(use-package! org-passwords
  :config
  (setq org-passwords-file "~/.org-password/store.org.gpg"))
#+end_src
